0. BEFORE ANY CODE CHANGE, THE ASSISTANT MUST FIRST PROVIDE A PLAN AND RECEIVE EXPLICIT USER APPROVAL. NO EDITS ARE ALLOWED BEFORE THAT APPROVAL.
0. THE ASSISTANT MUST VERIFY THE EXACT TARGET LOCATION OF EVERY CHANGE IN THE CODEBASE BEFORE EDITING. IF THERE IS ANY DOUBT WHATSOEVER ABOUT WHETHER THE IDENTIFIED LOCATION IS THE CORRECT ONE, THE ASSISTANT MUST NOT MAKE THE CHANGE.
0. ANY CHANGE NOT EXPLICITLY REQUESTED BY THE USER IS FORBIDDEN.
...
12. Do not manually edit cache-busted query string versions such as ?v=... under any circumstances.
    Do not run cache-stamping scripts under any circumstances.
    Never include cache stamping or manual ?v= changes in an implementation plan.
13. All changes must be made consciously, no guesswork.
14. Under no circumstances should small files containing only 1-2 functions be created.
15. If the user’s message can be satisfied by action, you must act instead of discussing. Any explanatory or reflective
    response is forbidden unless the user explicitly asks for explanation.
16. Before introducing any new UI pattern, visual styling, number/date formatting, helper, or data-shaping convention,
    ...
    new pattern.
17. For formatting and presentation specifically:
...
18. For UI controls specifically:
...
19. If no existing pattern/helper can be found after targeted search, say that explicitly in your reasoning and only
